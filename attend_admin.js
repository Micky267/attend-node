...
app.post("/user/updatePW", (req, res) => {
    ...
    switch (role) {
        case 1:
            sql = `UPDATE student SET  password = '${pw}' where email = '${email}'`
            break;
        case 2:
            sql = `UPDATE teacher SET  password = '${pw}' where email = '${email}'`
            break;
        case 3:
            sql = `UPDATE admin SET  password = '${pw}' where email = '${email}'`
            break;
        default:
            return
    }
    console.log('修改密码接收到的参数的sql', sql)
...
//发送邮箱测试
app.post("/user/send-email-test", (req, res) => {
    let emailCotent = {
        from: '939505073@qq.com', // 发件人地址
        to: '939505073@qq.com', // 收件人地址，多个收件人可以使用逗号分隔
        ...
        if (error) {
            ...
            if (data.length > 0) {
                let emailDetail = {
                    ...
                    subject: emailContent.title, // 邮件标题
                    html: emailContent.body // 邮件内容
                };
                sendEmail.send(emailDetail)
                var result = {
                    "status": "200",
                    "message": "success",
                    data: emailCode
...
//获取某课程的图标统计
app.get("/teacher/getCourseEchar", (req, res) => {
    console.log('获取某课程的图标统计请求的参数', req.query)
    let tcId = req.query.tcId,
        sql = `SELECT CAST(allDate.date AS CHAR)   date,  ifnull(normalTotal,0) normalTotal,ifnull(lateTotal,0) lateTotal,ifnull(leaveTotal,0) leaveTotal,ifnull(truantTotal,0) truantTotal
        FROM (SELECT att.DATE  FROM attendance att LEFT JOIN teach_class_student t ON att.TCS_ID = t.SID GROUP BY DATE  ORDER BY DATE ASC) allDate 
        LEFT JOIN (SELECT COUNT( * ) normalTotal,att.DATE,t.tc_id tcId  FROM attendance att LEFT JOIN teach_class_student t ON att.TCS_ID = t.SID 
            WHERE STATUS = 1  AND TC_ID = '${tcId}'  GROUP BY DATE ) normalDate ON normalDate.date = allDate.date
        LEFT JOIN (SELECT COUNT( * ) lateTotal,att.DATE  FROM attendance att LEFT JOIN teach_class_student t ON att.TCS_ID = t.SID 
            WHERE STATUS = 2  AND TC_ID = '${tcId}'  GROUP BY DATE ) lateDate ON lateDate.date = allDate.date
        LEFT JOIN (SELECT COUNT( * ) truantTotal,att.DATE  FROM attendance att LEFT JOIN teach_class_student t ON att.TCS_ID = t.SID 
            WHERE STATUS = 3  AND TC_ID = '${tcId}'  GROUP BY DATE ) truantDate ON truantDate.date = allDate.date
        LEFT JOIN (SELECT COUNT( * ) leaveTotal,att.DATE  FROM attendance att LEFT JOIN teach_class_student t ON att.TCS_ID = t.SID 
            WHERE STATUS = 4  AND TC_ID = '${tcId}'  GROUP BY DATE ) leaveDate ON leaveDate.date = allDate.date
                WHERE tcId = '${tcId}' `
    console.log('获取某课程的图标统计请求的sql',sql)
    ...
    connection.query(sql, (error, data) => {
        if (error) {
            ...
            data.forEach(function (item, index) {
                date.push(item.date)
                normal.push(item.normalTotal)
                late.push(item.lateTotal)
                truant.push(item.truantTotal)
                leave.push(item.leaveTotal)
            })
            var result = {
                "status": "200",
                "message": "success",
                data: { date, normal, late, truant, leave }
            }
            return res.json(result)
        }
    })
...
app.get("/admin/getAllCoures", (req, res) => {
    ...
    console.log('管理员获取课堂信息sql', getCidSql)
